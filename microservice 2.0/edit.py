...
while True:
    ...
    if is_it_time == "time":
        ...
        manuscript_file = f"manuscript/{manuscript_file_name}.txt"
        ...
        with open(manuscript_file, 'r') as manuscript:
            ...
            with open(manuscript_file, 'w') as manuscript:
                ...
                for line in manuscript_lines:
                    # ADD // HAS BEEN UPDATED 
                    if edit_integer == 1:
                        add_file = open("add.txt", 'r')
                        text = add_file.readline()
                        add_file.close()
                        if line_number == line_index:
                            manuscript_lines.insert(line_number + 1, f'{text} \n')
                        line_index += 1
